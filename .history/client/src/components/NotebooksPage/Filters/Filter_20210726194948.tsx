...
const Filter: FunctionComponent<Props> = (props) => {
  ...
  console.log({ filterInput, filterOptions });
              ...
              <div className="border-end-0 form-control form-control-sm bg-white">
                <Select
                  ...
                  noOptionsMessage={(option) => {
                    if (option.inputValue.length < 2) {
                      return "Start typing...";
                    }
                    if (
                      autoCompleteSuggestions.length === 0 &&
                      option.inputValue.length >= 2
                    ) {
                      return "No Results!";
                    }
                  }}
                  ...
                  onInputChange={(inputValue) => {
                    if (inputValue.length >= 2) {
                      fetchAutocompleteSuggestions(type, inputValue);
                      setFilterInput({ ...filterInput, value: inputValue });
                    }
                  }}
                  onChange={(option, { action }) => {
                    setFilterInput({ ...filterInput, value: option.value });
                  }}
                />
              </div>
            ) : (
    ...
    >
      <InputGroup className="mb-3">
        ...
        <AsyncTypeahead
          ...
          options={autoCompleteSuggestions}
          placeholder="Search..."
          renderMenuItemChildren={(option, props) => <span>{option}</span>}
        />
        <InputGroup.Text
