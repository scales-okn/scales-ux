        ...
        return (
          <>
            {filterOptions?.autocomplete ? (
              <Select options={autoCompleteSuggestions} onChange={(event) => fetchAutocompleteSuggestions(type, event.target.value)} />
            ) : (
              <FormControl
                className="border-end-0"
                size="sm"
                onChange={(event) => setFilterInput({ ...filterInput, event?.target.value })}
                value={value}
              />
            )}
          </>
