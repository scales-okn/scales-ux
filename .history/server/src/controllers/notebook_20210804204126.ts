...
export const update = async (req: Request, res: Response) => {
  try {
    ...
    const notebook = await sequelize.models.Notebook.findOne({ where: { id } });
    const notebookVersions = sequelize.models.Notebook.getVersions({ where: { id } });

    return res.send_ok("Notebook has been updated!", { notebook, notebookVersions });
  } catch (error) {
    console.log(error);
...
export const deleteNotebook = async (req: Request, res: Response) => {
  try {
    const id = req.params.notebookId;
    const result = await sequelize.models.Notebook.destroy({ where: { id } });
    if (result) {
